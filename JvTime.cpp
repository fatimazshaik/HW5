...
JvTime::JvTime(const char* timeString) {
    ...
    if ((timeString == NULL) || (strlen(timeString) != strlen(zero_str)))
    {
        this->validTimeFormat = false;
    }

    //Rewrote part of the JvTime function from class; Checks if the string
    //provided has the correct symbols in the right areas a little more
    //efficiently.
    for(int i = 0; i <= 23; i++)
    {
        if((i == 4 || i == 7) && (timeString[i] != '-')) {
            this->validTimeFormat = false;
    ...
    if (this->validTimeFormat == true)
    {
        sscanf(timeString, "%4d-%2d-%2dT%2d:%2d:%2d+%4s",
               &(this->year), &(this->month), &(this->day),
               &(this->hour), &(this->minute), &(this->second),
               (this->tail4));
    }
